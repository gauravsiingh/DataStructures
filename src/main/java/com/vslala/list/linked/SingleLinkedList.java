...
public class SingleLinkedList<T> implements List<T> {
    ...
    public boolean contains(Object o) {
        ...
        while (null != tempNode)
        {
            if (tempNode.getData().equals(checkedObj))
                return true;
    ...
    public <T1> T1[] toArray(T1[] a) {
        SingleNode<T> tempNode = headNode;
        if (size < a.length)
        {
            int index = 0;
            while (tempNode != null)
            {
                a[index++] = (T1) tempNode.getData();
                tempNode = tempNode.getNext();
            }
            return a;
        }

        T1[] newArr = (T1[]) Array.newInstance(a.getClass().getComponentType(), size());
        for (int i = 0; i<size(); i++) {
            newArr[i] = (T1) tempNode.getData();
            tempNode = tempNode.getNext();
        }
        return newArr;
    }
